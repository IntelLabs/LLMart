...
def attack(
    ...
    for step_idx in range(num_steps):
        ...
        with torch.no_grad():
            ...
            # Extract claim
            pattern = r"Document: (.*)"
            adv_doc = re.search(pattern, adv_user_content, re.DOTALL).group(1).strip()  # type: ignore
            ...
            _, adv_prob, _, _ = scorer.score(docs=[adv_doc], claims=[claim])
            adv_prob = adv_prob[0]  # type: ignore
            print(
                f"{step_idx = }, {model_loss = :0.4f}, {loss = :0.4f}, {adv_prob = :0.4f}, {adv_doc = }"
            )
